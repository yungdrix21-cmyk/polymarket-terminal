...
const T = {
  ...
  purple: '#9b7dff',
  purpleDim: 'rgba(155,125,255,0.12)',
  yellow: '#f5c842',
...
const Icon = ({ name, size = 16, color = 'currentColor', strokeWidth = 1.6 }) => {
  const paths = {
    ...
    copy2: <><polyline points="20 6 9 17 4 12"/></>,
...
// ─── Stat Card ─────────────────────────────────────────────────────────────
function StatCard({ label, value, color, icon, sub, trend }) {
  return (
    <div style={{
      background: T.bgCard,
      borderRadius: 14,
      ...
      <div style={{ color: color, fontSize: 22, fontWeight: 700, fontFamily: T.mono, letterSpacing: '-0.5px' }}>{value}</div>
    ...
    <div style={{ padding: '28px 28px 40px', overflowY: 'auto', flex: 1 }}>
      ...
      <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ color: T.text2, margin: '4px 0 0', fontSize: 13 }}>{user?.email || 'trader@example.com'}</p>
        </div>
      ...
      <div style={{ background: T.bgCard, borderRadius: 16, border: `1px solid ${T.border}`, padding: '20px 24px', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ color: T.blue }}><Icon name="arrowUp" size={16} color={T.blue} /></div>
      ...
      <div style={{ background: T.bgCard, borderRadius: 16, border: `1px solid ${T.border}`, overflow: 'hidden' }}>
        ...
        {recentDeposits.map((dep, i) => (
          <div key={i} style={{
            ...
            padding: '14px 24px',
            borderBottom: i < recentDeposits.length - 1 ? `1px solid ${T.border}` : 'none',
            ...
            return (
              ...
              >
                ...
                <div style={{ marginTop: 6, fontSize: 11, color: T.text2, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <Icon name="chart" size={10} color={T.text2} />
          ...
          <>
            <div style={{ padding: '16px 24px', borderBottom: `1px solid ${T.border}`, background: T.bg1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: T.text2, marginTop: 3, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Icon name="clock" size={11} color={T.text2} />
                  {selectedLive.timeframe} · Polymarket Crypto
                </div>
              </div>
            ...
            <div style={{ flex: 1, padding: '24px', overflowY: 'auto' }}>
              <div style={{ fontSize: 10, color: T.text2, marginBottom: 8, letterSpacing: '0.08em', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Icon name="chart" size={12} color={T.text2} />
...
function DepositsPage({ onDepositSuccess }) {
  ...
  const handleConfirmDeposit = () => {
    ...
    const newDeposit = { id: Date.now(), crypto: selectedCrypto.symbol, amount: parseFloat(amount), date: 'Just now', status: 'Pending' }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
        {cryptos.map(c => (
          ...
          >
            ...
            <div style={{ marginTop: 12, display: 'inline-block', padding: '4px 14px', borderRadius: 20, background: `${c.color}15`, color: c.color, fontSize: 11, fontWeight: 600 }}>
              Deposit
            </div>
          </div>
        ))}
      ...
      {selectedCrypto && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: T.bg2, padding: 32, borderRadius: 22, width: '92%', maxWidth: 460, position: 'relative', border: `1px solid ${T.borderHi}` }}>
            <button onClick={() => { setSelectedCrypto(null); setAmount('') }}
              style={{ position: 'absolute', top: 20, right: 20, background: T.bg3, border: 'none', color: T.text1, borderRadius: 8, width: 32, height: 32, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <button onClick={() => { navigator.clipboard.writeText(selectedCrypto.address) }}
              style={{ width: '100%', padding: '12px', background: T.blueDim, color: T.blue, border: `1px solid ${T.blue}30`, borderRadius: 12, fontWeight: 600, marginBottom: 20, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            <button onClick={handleConfirmDeposit} disabled={!amount}
              style={{ width: '100%', padding: '15px', background: amount ? T.teal : T.bg3, color: amount ? '#0d0e14' : T.text2, border: 'none', borderRadius: 14, fontSize: 15, fontWeight: 700, cursor: amount ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>
              Confirm Deposit
            </button>
          </div>
        </div>
        ...
        {filtered.length > 0 ? filtered.map((trader, i) => (
          <div key={i} style={{ background: T.bgCard, borderRadius: 16, border: `1px solid ${T.border}`, padding: '20px 22px', transition: 'border-color 0.2s' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: T.yellow }}>
                <Icon name="star" size={12} color={T.yellow} />
                <span style={{ fontSize: 11, fontWeight: 600 }}>#{trader.rank}</span>
              </div>
            </div>
            ...
            <button style={{ width: '100%', padding: '11px', background: T.tealDim, color: T.teal, border: `1px solid ${T.teal}30`, borderRadius: 10, fontWeight: 600, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
...
export default function App() {
  const [user] = useState({ email: 'trader@example.com' })
  const [view, setView] = useState('dashboard')
  const [collapsed, setCollapsed] = useState(false)
  const [selected, setSelected] = useState(null)
  const [recentDeposits, setRecentDeposits] = useState(RECENT_DEPOSITS)

  const handleNewDeposit = d => setRecentDeposits(prev => [d, ...prev])
    ...
    return (
      <div onClick={() => setView(item.id)}
        ...
        style={{
          ...
          borderLeft: active && !collapsed ? `2px solid ${T.blue}` : '2px solid transparent',
    ...
    <div style={{ display: 'flex', height: '100vh', background: T.bg0, color: T.text0, fontFamily: T.font, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '12px 8px', borderTop: `1px solid ${T.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: collapsed ? '10px' : '10px 14px', justifyContent: collapsed ? 'center' : 'flex-start', borderRadius: 10, cursor: 'pointer', color: T.text2 }}
