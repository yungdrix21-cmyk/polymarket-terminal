  ...
  return (
    <div style={{ padding: '20px', overflowY: 'auto', flex: 1 }}>
      ...
      <div style={{ background: T.bgCard, borderRadius: 12, border: `1px solid ${T.border}`, padding: 16 }}>
        ...
        {recentDeposits.length > 0 ? recentDeposits.map((dep, i) => (
        ...
        )) : <div style={{ color: T.text2 }}>No deposits yet</div>}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ padding: '20px', color: T.text0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
        {cryptos.map(c => (
          <div key={c.symbol} onClick={() => setSelectedCrypto(c)} style={{ background: T.bgCard, padding: 20, borderRadius: 16, textAlign: 'center', cursor: 'pointer', border: `2px solid ${T.border}` }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: T.bgCard, padding: 28, borderRadius: 20, width: '92%', maxWidth: 460, position: 'relative' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <h3 style={{ margin: '12px 0 4px' }}>Deposit {selectedCrypto.symbol}</h3>
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ color: T.text2, fontSize: 13.5, marginBottom: 8 }}>Amount</div>
              <div style={{ display: 'flex', gap: 12 }}>
                <input type="number" step="0.000001" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" style={{ flex: 1, background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 12, padding: '16px', color: T.text0, fontSize: 20 }} />
            ...
            <button onClick={handleConfirm} disabled={!amount} style={{ width: '100%', padding: '17px', background: amount ? T.teal : T.blueDim, color: '#fff', border: 'none', borderRadius: 14, fontSize: 16.5, fontWeight: 700 }}>
              Confirm Deposit
            </button>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
        {filtered.length > 0 ? filtered.map((t, i) => (
          <div key={i} style={{ background: T.bgCard, borderRadius: 16, border: `1px solid ${T.border}`, padding: 20 }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: T.text0 }}>{t.name}</div>
            <div style={{ color: T.teal, fontSize: 22, fontWeight: 700, margin: '8px 0' }}>{t.profit}</div>
            <div style={{ color: T.text2 }}>{t.winRate} win rate • {t.followers} followers</div>
            <button style={{ width: '100%', marginTop: 16, padding: '14px', background: T.teal, color: '#fff', border: 'none', borderRadius: 12 }}>Copy Trader</button>
          </div>
          ...
          <div style={{ flex: 1, padding: '12px 10px' }}>
            {NAV_ITEMS.map(item => (
              <div 
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, marginBottom: 4, cursor: 'pointer', background: view === item.id ? T.bg3 : 'transparent', color: view === item.id ? T.text0 : T.text1 }}
              >
          ...
          <div style={{ padding: '0 10px 10px' }}>
            {BOTTOM_ITEMS.map(item => (
              <div 
                key={item.id}
                onClick={() => { setView(item.id); setShowMenu(false); }}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, marginBottom: 4, cursor: 'pointer', color: T.text1 }}
              >
                <span style={{ fontSize: 16 }}>{item.icon}</span>
                {item.label}
              </div>
