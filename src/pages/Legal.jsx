...
// ─────────────────────────────────────────────────────────────────────────────
// HOW TO ADD THIS PAGE TO YOUR PROJECT:
//
// App Router (most common in newer Next.js):
//   → Save as:  app/legal/page.jsx
//   → Visits:   yoursite.com/legal
//
// Pages Router (older Next.js):
//   → Save as:  pages/legal.jsx
//   → Visits:   yoursite.com/legal
//
// Then in Auth.jsx, update the footer Privacy Policy span to:
//   <a href="/legal" style={{ fontSize: 12, color: T.text2, cursor: 'pointer', textDecoration: 'none' }}>Privacy Policy</a>
// ─────────────────────────────────────────────────────────────────────────────
...
function Section({ title, children }) {
  return (
    <div style={{ marginBottom: 40 }}>
      <h2 style={{ fontSize: 20, fontWeight: 700, color: T.text0, margin: '0 0 16px 0', paddingBottom: 10, borderBottom: `1px solid ${T.border}` }}>
        {title}
      </h2>
      {children}
    </div>
...
function SubSection({ title, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <h3 style={{ fontSize: 15, fontWeight: 600, color: T.text0, margin: '0 0 10px 0' }}>{title}</h3>
      {children}
    </div>
  )
}
...
export default function LegalPage() {
  return (
    <div style={{ minHeight: '100vh', background: T.bg0, fontFamily: T.font, color: T.text0 }}>

      {/* Nav */}
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', height: 60, borderBottom: `1px solid ${T.border}`, background: 'rgba(13,14,20,0.95)', backdropFilter: 'blur(16px)', position: 'sticky', top: 0, zIndex: 100 }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px 100px' }}>
        ...
        {/* Page header */}
        <div style={{ marginBottom: 40 }}>
          <h1 style={{ fontSize: 32, fontWeight: 800, margin: '0 0 8px 0', color: T.text0 }}>Legal Information</h1>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 16 }}>
            {['Terms of service', 'Privacy policy', 'Risk disclosure', 'Cookie policy'].map(tab => (
              <div key={tab} style={{ padding: '6px 14px', borderRadius: 20, background: T.bg2, border: `1px solid ${T.border}`, fontSize: 12, color: T.text1 }}>{tab}</div>
            ))}
          </div>
        </div>
        ...
        <Section title="Risk Disclosure">
          ...
          <div style={{ background: T.red + '08', border: `1px solid ${T.red}30`, borderRadius: 12, padding: '16px 20px', marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 16 }}>⚠️</span>
              <span style={{ fontSize: 14, fontWeight: 700, color: T.red }}>IMPORTANT WARNING</span>
            </div>
          </div>
          ...
          <SubSection title="When NOT to Use the Platform">
            <InfoBox color={T.red} icon="✕">
              ...
              <BulletList color={T.red} items={[
                ...
                'Don\'t have a trading experience',
        ...
        {/* ── FULL DOCUMENTS ───────────────────────────────────────────── */}
        <Section title="Full Documents">
          <Table
            headers={['Document', 'Description']}
            rows={[
              ['Terms of Service', 'Full terms and conditions of use'],
              ['Privacy Policy', 'How we collect and process your data'],
              ['Risk Disclosure', 'Full risk information for trading on prediction markets'],
        ...
        <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 32, marginTop: 16 }}>
          <p style={{ fontSize: 12, color: T.text2, lineHeight: 1.7, margin: 0 }}>
            ...
            <span style={{ color: T.blue }}>Email: legal@polytrader.app</span><br />
            <span style={{ color: T.blue }}>Website: polytrader.app</span>
          </p>
          <div style={{ marginTop: 20, background: T.teal + '0a', border: `1px solid ${T.teal}25`, borderRadius: 10, padding: '12px 16px' }}>
